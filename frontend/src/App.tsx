...
import { AppShell, type MainNavView } from './components/layout/AppShell';
...
const MainApp: React.FC = () => {
  ...
  const [mainView, setMainView] = useState<MainNavView>('dashboard');

  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#0b0f19] flex flex-col items-center justify-center text-slate-400 gap-3">
        <Loader2 className="w-8 h-8 animate-spin text-cyan-500" />
        <p className="text-sm font-medium tracking-wide">Initializing Enermax Operator Session...</p>
      </div>
    );
  }
